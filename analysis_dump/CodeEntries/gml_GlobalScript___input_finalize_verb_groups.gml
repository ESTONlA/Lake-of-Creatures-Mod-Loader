...
{
    if (global.__input_initialization_phase != "__input_finalize_verb_groups")
    {
        exit;
    }
    ...
    repeat (array_length(global.__input_verb_group_array))
    {
        ...
        var _group_array = variable_struct_get(global.__input_group_to_verbs_dict, _group_name);
        ...
        repeat (array_length(_group_array))
        {
            ...
            if (!variable_struct_exists(global.__input_basic_verb_dict, _verb))
            {
                __input_error("Verb \"", _verb, "\" doesn't exist\nPlease check INPUT_VERB_GROUPS matches INPUT_DEFAULT_PROFILES");
            }
            if (variable_struct_exists(global.__input_verb_to_group_dict, _verb))
            {
                __input_error("Verb \"", _verb, "\" already has a group (", _group_name, ")");
            }
            variable_struct_set(global.__input_verb_to_group_dict, _verb, _group_name);
            _j++;
        }
        _i++;
    }
